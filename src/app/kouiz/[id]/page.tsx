    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await kouizDetails(token, id);
                if (response.success === true) {
                    setKouiz(response.data);
                    console.log(kouiz)
                } else {
                ...
                <div className="sm:pt-20 sm:pl-52 pt-20 md:pr-20 dark:bg-sBlue w-full pb-32 px-6">
                    <div className="flex lg:flex-row flex-col lg:items-center items-start justify-between">
                        <div className="flex flex-col">
                            <h1 className="text-3xl font-title font-bold text-pBrown">Kouiz.</h1>
                            <p className="font-body lg:my-4 my-2 text-md">Visualisez votre kouiz ici.</p>
                        </div>
                        <BackButton />
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center">
                        <div className="bg-pWhite dark:bg-pBlue border dark:border-0 rounded-xl space-y-2 flex flex-col w-full p-12">
                            {kouiz ? (
                                <>
                                    <div className="text-2xl text-pBrown font-black flex flex-wrap">
                                        {kouiz && kouiz.title}
                                    </div>
                                    <div>
                                        {kouiz && kouiz.description}
                                    </div>
                                    <div className="text-bold">
                                        Créé le {kouiz && kouiz.created_at} par {kouiz && kouiz.creator_name}
                                    </div>
                                    <div className="mt-8">
                                        <h2 className="text-xl font-bold">Questions :</h2>
                                        {kouiz && kouiz.questions && kouiz.questions.map((question, index) => (
