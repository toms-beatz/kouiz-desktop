...
import { UserRound, LayoutGrid, GalleryVerticalEnd, LogOut, Send, ShieldCheck, User, Users, Settings, Cog, ListChecks, Layers } from "lucide-react";
...
const AuthNavbar = () => {
    ...
    const showNavbar = ['/dashboard', '/kouiz', '/profile', '/answers', '/users'].some(path => pathname.startsWith(path));
                ...
                <aside className="fixed top-0 left-0 z-20 w-64 h-full transition-transform bg-pWhite dark:bg-pBlue border-r mt-14">
                    <div className="h-full overflow-y-auto bg-gray-50 dark:bg-gray-800 flex flex-col">
                        <ul className="font-medium *:py-4 *:px-6">
                            ...
                            {/* <li className={`${pathname.startsWith('/roles') ? "bg-pGray dark:bg-sBlue" : ""} hover:bg-pGray dark:hover:bg-sBlue`}>
                                <Link href="/roles" className="w-full h-full">
                                    <div className={`w-full h-full flex items-center font-title font-black text-xl  ${pathname.startsWith('/roles') ? 'text-pBrown' : ''}`}>
                                        <ShieldCheck /><span className="font-title font-medium text-sm ml-4">Rôles.</span>
                                    </div>
                                </Link>
                            </li> */}

                            {/* <li className={`${pathname.startsWith('/permissions') ? "bg-pGray dark:bg-sBlue" : ""} hover:bg-pGray dark:hover:bg-sBlue`}>
                                <Link href="/permissions" className="w-full h-full">
                                    <div className={`w-full h-full flex items-center font-title font-black text-xl  ${pathname.startsWith('/permissions') ? 'text-pBrown' : ''}`}>
                                        <ListChecks /><span className="font-title font-medium text-sm ml-4">Permissions.</span>
                                    </div>
                                </Link>
                            </li> */}

                            {/* <li className={`${pathname.startsWith('/categories') ? "bg-pGray dark:bg-sBlue" : ""} hover:bg-pGray dark:hover:bg-sBlue`}>
                                <Link href="/categories" className="w-full h-full">
                                    <div className={`w-full h-full flex items-center font-title font-black text-xl  ${pathname.startsWith('/categories') ? 'text-pBrown' : ''}`}>
                                        <Layers /><span className="font-title font-medium text-sm ml-4">Catégories.</span>
                                    </div>
                                </Link>
                            </li> */}

                            <li className={`${pathname.startsWith('/users') ? "bg-pGray dark:bg-sBlue" : ""} hover:bg-pGray dark:hover:bg-sBlue`}>
                                <Link href="/users" className="w-full h-full">
                                    <div className={`w-full h-full flex items-center font-title font-black text-xl  ${pathname.startsWith('/users') ? 'text-pBrown' : ''}`}>
                                        <Users /><span className="font-title font-medium text-sm ml-4">Utilisateurs.</span>
                                    </div>
                                </Link>
                            </li>
